...
parser.add_argument("--with-line-nb")
parser.add_argument("--path", default=".")
args = parser.parse_args()

if args.with_line_nb != None:
    line_nb = True

class_root_dir = args.path
...
matches = []
for root, dirnames, filenames in os.walk(class_root_dir):
    dirnames[:] = [d for d in dirnames if d not in ["thirdparty", "skeleton_retarget"]]
...
main_po = """
# LANGUAGE translation of the Spike Engine editor.
...
msgid ""
msgstr ""
"Project-Id-Version: Spike Engine editor\\n"
...
message_patterns = {
    re.compile(r'SRTR\("(?P<message>([^"\\]|\\.)*)"(, "(?P<context>([^"\\]|\\.)*)")?\)'): ExtractType.TEXT,
    re.compile(r'STTR\("(?P<message>([^"\\]|\\.)*)"(, "(?P<context>([^"\\]|\\.)*)")?\)'): ExtractType.TEXT,
    re.compile(r'STTRC\("(?P<message>([^"\\]|\\.)*)"\)'): ExtractType.TEXT,
    re.compile(
        r'STTRN\("(?P<message>([^"\\]|\\.)*)", "(?P<plural_message>([^"\\]|\\.)*)",[^,)]+?(, "(?P<context>([^"\\]|\\.)*)")?\)'
    ): ExtractType.TEXT,
    re.compile(
        r'SRTRN\("(?P<message>([^"\\]|\\.)*)", "(?P<plural_message>([^"\\]|\\.)*)",[^,)]+?(, "(?P<context>([^"\\]|\\.)*)")?\)'
    ): ExtractType.TEXT,
...
def _add_message(msg, msg_plural, msgctx, location, translator_comment):
    ...
    if not message:
        ...
        message.msgctxt = msgctx if msgctx != "" else "Spike"
        message.locations = []
        message.comments = []
