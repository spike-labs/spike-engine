...
RendererSceneRenderGLES3 *RendererSceneRenderGLES3::singleton = nullptr;

RenderGeometryInstance *RendererSceneRenderGLES3::geometry_instance_create(RID p_base) {
...
uint32_t RendererSceneRenderGLES3::geometry_instance_get_pair_mask() {
	return (1 << RS::INSTANCE_LIGHT);
}

void RendererSceneRenderGLES3::GeometryInstanceGLES3::pair_light_instances(const RID *p_light_instances, uint32_t p_light_instance_count) {
...
void RendererSceneRenderGLES3::geometry_instance_free(RenderGeometryInstance *p_geometry_instance) {
...
void RendererSceneRenderGLES3::GeometryInstanceGLES3::_mark_dirty() {
	...
	while (surf) {
		GeometryInstanceSurface *next = surf->next;
		RendererSceneRenderGLES3::get_singleton()->geometry_instance_surface_alloc.free(surf);
		surf = next;
	}

	surface_caches = nullptr;

	RendererSceneRenderGLES3::get_singleton()->geometry_instance_dirty_list.add(&dirty_list_element);
}

void RendererSceneRenderGLES3::GeometryInstanceGLES3::set_use_lightmap(RID p_lightmap_instance, const Rect2 &p_lightmap_uv_scale, int p_lightmap_slice_index) {
}

void RendererSceneRenderGLES3::GeometryInstanceGLES3::set_lightmap_capture(const Color *p_sh9) {
}

void RendererSceneRenderGLES3::_update_dirty_geometry_instances() {
	while (geometry_instance_dirty_list.first()) {
		_geometry_instance_update(geometry_instance_dirty_list.first()->self());
	}
}

void RendererSceneRenderGLES3::_geometry_instance_dependency_changed(Dependency::DependencyChangedNotification p_notification, DependencyTracker *p_tracker) {
	switch (p_notification) {
		...
		case Dependency::DEPENDENCY_CHANGED_MULTIMESH_VISIBLE_INSTANCES: {
			...
			if (ginstance->data->base_type == RS::INSTANCE_MULTIMESH) {
				ginstance->instance_count = OPENGL3::MeshStorage::get_singleton()->multimesh_get_instances_to_draw(ginstance->data->base);
			}
		} break;
		default: {
...
void RendererSceneRenderGLES3::_geometry_instance_dependency_deleted(const RID &p_dependency, DependencyTracker *p_tracker) {
	static_cast<RenderGeometryInstance *>(p_tracker->userdata)->_mark_dirty();
}

void RendererSceneRenderGLES3::_geometry_instance_add_surface_with_material(GeometryInstanceGLES3 *ginstance, uint32_t p_surface, GLES3::SceneMaterialData *p_material, uint32_t p_material_id, uint32_t p_shader_id, RID p_mesh) {
	OPENGL3::MeshStorage *mesh_storage = OPENGL3::MeshStorage::get_singleton();
...
void RendererSceneRenderGLES3::_geometry_instance_add_surface_with_material_chain(GeometryInstanceGLES3 *ginstance, uint32_t p_surface, GLES3::SceneMaterialData *p_material_data, RID p_mat_src, RID p_mesh) {
...
void RendererSceneRenderGLES3::_geometry_instance_add_surface(GeometryInstanceGLES3 *ginstance, uint32_t p_surface, RID p_material, RID p_mesh) {
	GLES3::MaterialStorage *material_storage = GLES3::MaterialStorage::get_singleton();
	RID m_src;
...
void RendererSceneRenderGLES3::_geometry_instance_update(RenderGeometryInstance *p_geometry_instance) {
	OPENGL3::MeshStorage *mesh_storage = OPENGL3::MeshStorage::get_singleton();
	OPENGL3::ParticlesStorage *particles_storage = OPENGL3::ParticlesStorage::get_singleton();
...
void RendererSceneRenderGLES3::_free_sky_data(Sky *p_sky) {
	if (p_sky->radiance != 0) {
...
RID RendererSceneRenderGLES3::sky_allocate() {
	return sky_owner.allocate_rid();
}

void RendererSceneRenderGLES3::sky_initialize(RID p_rid) {
	sky_owner.initialize_rid(p_rid);
}

void RendererSceneRenderGLES3::sky_set_radiance_size(RID p_sky, int p_radiance_size) {
...
void RendererSceneRenderGLES3::sky_set_mode(RID p_sky, RS::SkyMode p_mode) {
	Sky *sky = sky_owner.get_or_null(p_sky);
	ERR_FAIL_COND(!sky);
...
void RendererSceneRenderGLES3::sky_set_material(RID p_sky, RID p_material) {
	Sky *sky = sky_owner.get_or_null(p_sky);
	ERR_FAIL_COND(!sky);
...
float RendererSceneRenderGLES3::sky_get_baked_exposure(RID p_sky) const {
	Sky *sky = sky_owner.get_or_null(p_sky);
...
void RendererSceneRenderGLES3::_invalidate_sky(Sky *p_sky) {
	if (!p_sky->dirty) {
		p_sky->dirty = true;
...
void RendererSceneRenderGLES3::_update_dirty_skys() {
...
void RendererSceneRenderGLES3::_setup_sky(const RenderDataGLES3 *p_render_data, const PagedArray<RID> &p_lights, const Projection &p_projection, const Transform3D &p_transform, const Size2i p_screen_size) {
...
void RendererSceneRenderGLES3::_draw_sky(RID p_env, const Projection &p_projection, const Transform3D &p_transform, float p_luminance_multiplier) {
	GLES3::MaterialStorage *material_storage = GLES3::MaterialStorage::get_singleton();
...
void RendererSceneRenderGLES3::_update_sky_radiance(RID p_env, const Projection &p_projection, const Transform3D &p_transform, float p_luminance_multiplier) {
	GLES3::MaterialStorage *material_storage = GLES3::MaterialStorage::get_singleton();
	ERR_FAIL_COND(p_env.is_null());
...
// Implemented at `drivers/gles3/rasterizer_scene_gles3.cpp`
...
void RendererSceneRenderGLES3::_filter_sky_radiance(Sky *p_sky, int p_base_layer) {
	GLES3::MaterialStorage *material_storage = GLES3::MaterialStorage::get_singleton();
...
Ref<Image> RendererSceneRenderGLES3::sky_bake_panorama(RID p_sky, float p_energy, bool p_bake_irradiance, const Size2i &p_size) {
	return Ref<Image>();
}

/* ENVIRONMENT API */

void RendererSceneRenderGLES3::environment_glow_set_use_bicubic_upscale(bool p_enable) {
	glow_bicubic_upscale = p_enable;
}

void RendererSceneRenderGLES3::environment_set_ssr_roughness_quality(RS::EnvironmentSSRRoughnessQuality p_quality) {
}

void RendererSceneRenderGLES3::environment_set_ssao_quality(RS::EnvironmentSSAOQuality p_quality, bool p_half_size, float p_adaptive_target, int p_blur_passes, float p_fadeout_from, float p_fadeout_to) {
}

void RendererSceneRenderGLES3::environment_set_ssil_quality(RS::EnvironmentSSILQuality p_quality, bool p_half_size, float p_adaptive_target, int p_blur_passes, float p_fadeout_from, float p_fadeout_to) {
}

void RendererSceneRenderGLES3::environment_set_sdfgi_ray_count(RS::EnvironmentSDFGIRayCount p_ray_count) {
}

void RendererSceneRenderGLES3::environment_set_sdfgi_frames_to_converge(RS::EnvironmentSDFGIFramesToConverge p_frames) {
}

void RendererSceneRenderGLES3::environment_set_sdfgi_frames_to_update_light(RS::EnvironmentSDFGIFramesToUpdateLight p_update) {
}

void RendererSceneRenderGLES3::environment_set_volumetric_fog_volume_size(int p_size, int p_depth) {
}

void RendererSceneRenderGLES3::environment_set_volumetric_fog_filter_active(bool p_enable) {
}

Ref<Image> RendererSceneRenderGLES3::environment_bake_panorama(RID p_env, bool p_bake_irradiance, const Size2i &p_size) {
	return Ref<Image>();
}

void RendererSceneRenderGLES3::positional_soft_shadow_filter_set_quality(RS::ShadowQuality p_quality) {
}

void RendererSceneRenderGLES3::directional_soft_shadow_filter_set_quality(RS::ShadowQuality p_quality) {
}

RID RendererSceneRenderGLES3::fog_volume_instance_create(RID p_fog_volume) {
	return RID();
}

void RendererSceneRenderGLES3::fog_volume_instance_set_transform(RID p_fog_volume_instance, const Transform3D &p_transform) {
}

void RendererSceneRenderGLES3::fog_volume_instance_set_active(RID p_fog_volume_instance, bool p_active) {
}

RID RendererSceneRenderGLES3::fog_volume_instance_get_volume(RID p_fog_volume_instance) const {
	return RID();
}

Vector3 RendererSceneRenderGLES3::fog_volume_instance_get_position(RID p_fog_volume_instance) const {
	return Vector3();
}

RID RendererSceneRenderGLES3::voxel_gi_instance_create(RID p_voxel_gi) {
	return RID();
}

void RendererSceneRenderGLES3::voxel_gi_instance_set_transform_to_data(RID p_probe, const Transform3D &p_xform) {
}

bool RendererSceneRenderGLES3::voxel_gi_needs_update(RID p_probe) const {
	return false;
}

void RendererSceneRenderGLES3::voxel_gi_update(RID p_probe, bool p_update_light_instances, const Vector<RID> &p_light_instances, const PagedArray<RenderGeometryInstance *> &p_dynamic_objects) {
}

void RendererSceneRenderGLES3::voxel_gi_set_quality(RS::VoxelGIQuality) {
}

void RendererSceneRenderGLES3::_fill_render_list(RenderListType p_render_list, const RenderDataGLES3 *p_render_data, PassMode p_pass_mode, bool p_append) {
	OPENGL3::MeshStorage *mesh_storage = OPENGL3::MeshStorage::get_singleton();
...
void RendererSceneRenderGLES3::_setup_environment(const RenderDataGLES3 *p_render_data, bool p_no_fog, const Size2i &p_screen_size, bool p_flip_y, const Color &p_default_bg_color, bool p_pancake_shadows) {
	Projection correction;
...
void RendererSceneRenderGLES3::_setup_lights(const RenderDataGLES3 *p_render_data, bool p_using_shadows, uint32_t &r_directional_light_count, uint32_t &r_omni_light_count, uint32_t &r_spot_light_count) {
	GLES3::LightStorage *light_storage = GLES3::LightStorage::get_singleton();
	GLES3::Config *config = GLES3::Config::get_singleton();
...
void RendererSceneRenderGLES3::render_scene(const Ref<RenderSceneBuffers> &p_render_buffers, const CameraData *p_camera_data, const CameraData *p_prev_camera_data, const PagedArray<RenderGeometryInstance *> &p_instances, const PagedArray<RID> &p_lights, const PagedArray<RID> &p_reflection_probes, const PagedArray<RID> &p_voxel_gi_instances, const PagedArray<RID> &p_decals, const PagedArray<RID> &p_lightmaps, const PagedArray<RID> &p_fog_volumes, RID p_environment, RID p_camera_attributes, RID p_shadow_atlas, RID p_occluder_debug_tex, RID p_reflection_atlas, RID p_reflection_probe, int p_reflection_probe_pass, float p_screen_mesh_lod_threshold, const RenderShadowData *p_render_shadows, int p_render_shadow_count, const RenderSDFGIData *p_render_sdfgi_regions, int p_render_sdfgi_region_count, const RenderSDFGIUpdateData *p_sdfgi_update_data, RenderingMethod::RenderInfo *r_render_info) {
...
void RendererSceneRenderGLES3::_render_list_template(RenderListParameters *p_params, const RenderDataGLES3 *p_render_data, uint32_t p_from_element, uint32_t p_to_element, bool p_alpha_pass) {
	OPENGL3::MeshStorage *mesh_storage = OPENGL3::MeshStorage::get_singleton();
	OPENGL3::ParticlesStorage *particles_storage = OPENGL3::ParticlesStorage::get_singleton();
	GLES3::MaterialStorage *material_storage = GLES3::MaterialStorage::get_singleton();
	GLES3::TextureStorage *texture_storage = GLES3::TextureStorage::get_singleton();
...
void RendererSceneRenderGLES3::render_material(const Transform3D &p_cam_transform, const Projection &p_cam_projection, bool p_cam_orthogonal, const PagedArray<RenderGeometryInstance *> &p_instances, RID p_framebuffer, const Rect2i &p_region) {
}

void RendererSceneRenderGLES3::render_particle_collider_heightfield(RID p_collider, const Transform3D &p_transform, const PagedArray<RenderGeometryInstance *> &p_instances) {
	OPENGL3::ParticlesStorage *particles_storage = OPENGL3::ParticlesStorage::get_singleton();
...
void RendererSceneRenderGLES3::set_time(double p_time, double p_step) {
	time = p_time;
	time_step = p_step;
}

void RendererSceneRenderGLES3::set_debug_draw_mode(RS::ViewportDebugDraw p_debug_draw) {
	debug_draw = p_debug_draw;
}

Ref<RenderSceneBuffers> RendererSceneRenderGLES3::render_buffers_create() {
	Ref<RenderSceneBuffersGLES3> rb;
	rb.instantiate();
	return rb;
}
...
void RendererSceneRenderGLES3::_render_buffers_debug_draw(Ref<RenderSceneBuffersGLES3> p_render_buffers, RID p_shadow_atlas, RID p_occlusion_buffer) {
}

void RendererSceneRenderGLES3::gi_set_use_half_resolution(bool p_enable) {
}

void RendererSceneRenderGLES3::screen_space_roughness_limiter_set_active(bool p_enable, float p_amount, float p_curve) {
}

bool RendererSceneRenderGLES3::screen_space_roughness_limiter_is_active() const {
	return false;
}

void RendererSceneRenderGLES3::sub_surface_scattering_set_quality(RS::SubSurfaceScatteringQuality p_quality) {
}

void RendererSceneRenderGLES3::sub_surface_scattering_set_scale(float p_scale, float p_depth_scale) {
}

TypedArray<Image> RendererSceneRenderGLES3::bake_render_uv2(RID p_base, const TypedArray<RID> &p_material_overrides, const Size2i &p_image_size) {
	return TypedArray<Image>();
}

bool RendererSceneRenderGLES3::free(RID p_rid) {
...
void RendererSceneRenderGLES3::update() {
	_update_dirty_skys();
}

void RendererSceneRenderGLES3::sdfgi_set_debug_probe_select(const Vector3 &p_position, const Vector3 &p_dir) {
}

void RendererSceneRenderGLES3::decals_set_filter(RS::DecalFilter p_filter) {
}

void RendererSceneRenderGLES3::light_projectors_set_filter(RS::LightProjectorFilter p_filter) {
}

RendererSceneRenderGLES3::RendererSceneRenderGLES3() {
	singleton = this;
...

	{
		...
		config->max_renderable_lights = MIN(config->max_renderable_lights, config->max_uniform_buffer_size / (int)sizeof(RendererSceneRenderGLES3::LightData));
