...
void RendererCompositorGLES3::begin_frame(double frame_step) {
	frame++;
	delta = frame_step;
	...
	OPENGL3::Utilities *utils = OPENGL3::Utilities::get_singleton();
	utils->_capture_timestamps_begin();

	//scene->iteration();
}

void RendererCompositorGLES3::end_frame(bool p_swap_buffers) {
	if (p_swap_buffers) {
...
void RendererCompositorGLES3::initialize() {
	print_line("OpenGL Renderer: " + RS::get_singleton()->get_video_adapter_name());
}

void RendererCompositorGLES3::finalize() {
	memdelete(scene);
	memdelete(canvas);
	memdelete(gi);
	memdelete(fog);
...
RendererCompositorGLES3::RendererCompositorGLES3() {
#ifdef GLAD_ENABLED
	if (!gladLoaderLoadGL()) {
	...
	utilities = memnew(OPENGL3::Utilities);
	texture_storage = memnew(GLES3::TextureStorage);
	material_storage = memnew(GLES3::MaterialStorage);
	mesh_storage = memnew(OPENGL3::MeshStorage);
	particles_storage = memnew(OPENGL3::ParticlesStorage);
	light_storage = memnew(GLES3::LightStorage);
	copy_effects = memnew(GLES3::CopyEffects);
	gi = memnew(GLES3::GI);
	fog = memnew(GLES3::Fog);
	canvas = memnew(RendererCanvasRenderGLES3());
	scene = memnew(RendererSceneRenderGLES3());
}

RendererCompositorGLES3::~RendererCompositorGLES3() {
}

void RendererCompositorGLES3::prepare_for_blitting_render_targets() {
...
void RendererCompositorGLES3::_blit_render_target_to_screen(RID p_render_target, DisplayServer::WindowID p_screen, const Rect2 &p_screen_rect, uint32_t p_layer) {
...
void RendererCompositorGLES3::blit_render_targets_to_screen(DisplayServer::WindowID p_screen, const BlitToScreen *p_render_targets, int p_amount) {
...
void RendererCompositorGLES3::set_boot_image(const Ref<Image> &p_image, const Color &p_color, bool p_scale, bool p_use_filter) {
