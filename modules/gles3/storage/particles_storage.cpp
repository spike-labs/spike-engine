...
void ParticlesStorage::_particles_process(Particles *p_particles, double p_delta) {
	...
	GLES3::ParticleProcessMaterialData *m = static_cast<GLES3::ParticleProcessMaterialData *>(material_storage->material_get_data(p_particles->process_material, RS::SHADER_PARTICLES));
	if (!m) {
		m = static_cast<GLES3::ParticleProcessMaterialData *>(material_storage->material_get_data(particles_shader.default_material, RS::SHADER_PARTICLES));
	}
